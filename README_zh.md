...
## 介绍
Flame 是一个开源的大型视觉语言模型（VLM），专为前端代码生成而设计。它旨在通过利用多模态学习技术，弥合 UI 设计稿与可执行前端代码之间的差距。本仓库包含了 Flame 的数据准备管道、训练过程和评估管道的完整实现，如我们的研究论文所述。通过结合自动化数据合成、自反思训练和基准测试，Flame 在视觉到代码的任务中实现了最先进的性能，特别是在基于 React 的开发中。
